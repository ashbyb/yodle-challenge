...
class Circuit(JuggleFestEntityBase):
	...
	def __init__(self, number, h, e, p, max_jugglers=0):
		...
		self.e = e
		self.p = p
		self.jugglers = [] # Default to empty until we can reference the circuit objects (default: []).
		self.max_jugglers = max_jugglers # Prevent adding participants until we know how many we can take (default: 0).

	def __repr__(self):
		return "<Circuit #%d: H:%d, E:%d, P:%d, Max:%d, Jugglers[%d]: %s>" % (self.num, self.h, self.e, 
			    self.p, self.max_jugglers, len(self.jugglers), ', '.join(['J'+str(j[0].num) for j in self.jugglers]))
...
class Juggler(JuggleFestEntityBase):
	...
	def __repr__(self):
		return "<Juggler #%d: H:%d, E:%d, P:%d, Prefs[%d]: %s>" % (self.num, self.h, self.e, self.p, 
				len(self.preferences), ', '.join(['C'+str(pref.num) for pref in self.preferences]))
